...
<div class="container">
    ...
    <div class="result-panel">
        ...
        <div id="solution" class="solution-box">請調整參數...</div>
    </div>
</div>
...
<script>
    ...
    function calculate() {
        ...
        if (a !== 0) {
            // 一元二次方程式
            html += `<span class="type-badge">一元二次方程式</span>`;
            const D = b * b - 4 * a * c;
            const den = 2 * a;
            html += `<div style="color:#64748b; font-size:0.9rem">判別式 D = ${D}</div>`;

            if (D === 0) {
                html += `<div class="solution-main">重根：<br>x = ` + renderFraction(-b, den) + `</div>`;
            } else if (D > 0) {
                const [out, ins] = simplifyRadical(D);
                html += `<div class="solution-main">相異實根：<br>` + formatSolution(-b, out, ins, den, "") + `</div>`;
            } else {
                const [out, ins] = simplifyRadical(-D);
                html += `<div class="solution-main">共軛虛根：<br>` + formatSolution(-b, out, ins, den, "i") + `</div>`;
            }
        } else if (b !== 0) {
            // 一元一次方程式 (bx + c = 0 -> x = -c/b)
            html += `<span class="type-badge" style="background:#fef3c7; color:#b45309">一元一次方程式</span>`;
            html += `<div class="solution-main">唯一實根：<br>x = ` + renderFraction(-c, b) + `</div>`;
        } else {
            // 恆等式或矛盾
            if (c === 0) {
                html += `<div style="color:var(--accent-green)">恆等式：0 = 0 (無限多解)</div>`;
            } else {
                html += `<div style="color:var(--accent-red)">矛盾：${c} = 0 (無解)</div>`;
            }
        }
    ...
    function formatSolution(minusB, out, ins, den, unit) {
        if (ins === 1 && unit === "") {
            ...
            return `x₁ = ${renderFraction(x1_num, den)}<br>x₂ = ${renderFraction(x2_num, den)}`;
        }
        ...
        const rootPart = (ins === 1) ? "" : `√${ins}`;
        const numStr = `${b_final} ± ${outStr}${rootPart}${unit}`;
